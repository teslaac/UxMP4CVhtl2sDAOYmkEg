...
class User_model extends CI_Model{
	...
	*/
	...
	public function add_user($udata, $odata, $id = NULL, $foreign_key, $table,$session_id = ''){
		$sch_id = $_SESSION['sch_id'];
		$sch_short_name = $_SESSION['sch_id'];
		if(empty($session_id)){
			$session_id = $this->Corem->active_term_session('session_id', $sch_id);
		}
		...
		$acadah_user_id = $this->db->insert_id();
		// $user_id = ;
		if(empty($acadah_user_id)){
			die('Am Lost in Transmission');
		}
		//Let Me quickly Update The user code of the user added now
		if($table == 'acadah_prospective'){
			$lid = $this->user_id('prospective',$odata['ts_id']);
			$lid = $lid + 1;
			$user_id = "p".$odata['ts_id'].$lid;
		}else{
			$lid = $this->user_id('other');
			$lid = $lid;
			$user_id = $lid;
		}
		...
		$updata['acadah_user_id'] = $sch_short_name.$acadah_user_id;
		...
		$usercode_update = $this->db->update('acadah_users',$updata,array('id'=>$acadah_user_id));
		if(!$usercode_update){
			$this->db->trans_rollback();
		...
		if($table == 'acadah_student'){
			$sdata = array(
				'student_id' => $usercode,
				'class_details_id' => $odata['admission_class_details_id'],
				'sch_id' => $sch_id,
				'session_id' => $session_id
				);
			$this->db->insert('acadah_student_session',$sdata);
		}
		if(!$oquery){
			$this->db->trans_rollback();
        ...
        $login_details = array('username' => $usercode, 
        	'email' => $udata['email'] ,
        	'pass' => strtolower($udata['lname']), 
        	'success' => true);
	...
	/**
	* Function Written By Tes Sall
	* THis function returns the id assigned to the last added to the db
	*
	**/
	public function user_id($type,$ts_id = ''){
			$this->db->from('acadah_users');
			$this->db->where('sch_id',$_SESSION['sch_id']);
		if($type == 'prospective'){
			$this->db->join('acadah_prospective',"acadah_users.user_id = acadah_prospective.prospective_id");
			$this->db->where('ts_id',$ts_id);
		}elseif($type == 'smart_admission'){
			$this->db->join('acadah_prospective',"acadah_users.user_id = acadah_prospective.prospective_id");
			$this->db->where('ts_id',$ts_id);
			$this->db->where('acadah_prospective.admission_status','admitted');
		}elseif($type == 'other'){
...
//            $this->form_validation->set_rules('pbloodgroup', 'Bloodgroup', 'trim|required');
			...
			$this->form_validation->set_rules('phone', 'Phone Number', 'trim|min_length[10]|max_length[11]|numeric');
		...
		if($type == 'staff') {
			...
			$this->form_validation->set_rules('hostel_status', 'Day/Boarding', 'trim|required');
			$this->form_validation->set_rules('p_fullname', 'Parent Full Name', 'trim|required');
			$this->form_validation->set_rules('p_phone', 'Parent Phone No', 'trim|required');
			$this->form_validation->set_rules('p_full_address', 'Parent Address', 'trim|required');
		}
	...
	public function edit_user($table,$udata,$odata,$id,$user_id){
		...
		if($query){
			...
			}elseif($table == 'acadah_student'){
				if(empty($udata['status'])){
					$udata['status'] = 'active';
				}
			$this->db->update('acadah_student_session',array('status'=>$udata['status'],'class_details_id'=>$odata['admission_class_details_id']),array($user_id=>$id));
			}
			if(isset($_POST['role[]'])){
			$roles = $this->input->post('role[]');
...
//		die();
	...
	public function quick_register($no,$otable){
		...
		$updata = array('user_id'=>$usercode);
		$updata['acadah_user_id'] = $sch_id.$acadah_user_id;
		$this->db->set($updata);
		$this->db->where('id',$acadah_user_id);
