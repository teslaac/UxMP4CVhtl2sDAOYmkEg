...
//awonugaso
public function active_term_session($field = 'ts_id', $sch_id){
    ...
    //awonugaso
public function term_session($sch_id, $field = 'all', $active = '', $ts_id = null ){
        ...
        //if active 
        $active ? $this->db->where('ts_active', 1) : '';
        //for one tsid row
        $ts_id ? $this->db->where('sts.ts_id ', $ts_id) : '' ; 
        ...
        if( $field === 'all'){ //then all fields are needed
             ...
             $result = $query->result_array()[0][$field];
        }

        //if tsid return one row
        $ts_id ? $result =  $query->row_array() : '' ; 
        return $result;
    ...
    public function save_school($udata,$odata,$edit = FALSE){
        if($edit === FALSE){
            ...
            if($query) {
                ...
                $this->db->set('user_id', $odata['sch_id'].'1');
                $this->db->set('acadah_user_id', $usercode);
...
//        print_r($query->row_array());die();
    ...
    if($table == 'acadah_school_term_session' or $table == 'acadah_school_session'){
        if($table == 'acadah_school_session'){
            $field = 'session_active';
        }else{
            $field = 'ts_active';
        }
                $this->db->update("$table",array($field=>'inactive'),array('sch_id'=>$_SESSION['sch_id']));
        }

    if($edit === FALSE){
        $query = $this->db->on_duplicate($table,$data);
    }else{
        if($table == 'acadah_school_session'){
    ...
    public function get_details($table, $field, $id,$sch_id = FALSE,$count = FALSE ) {
         $this->db->from($table);
         $this->db->where($field, $id);
         if($sch_id === TRUE){
            $this->db->where('sch_id',$_SESSION['sch_id']);
         }
         if($table == 'acadah_school_scores'){
    ...
    **/
    ...
    public function status($table,$id,$status,$field,$key){
        if($table == 'acadah_school_term_session' or $table == 'acadah_school_session'){
            if($status == 'active'){
                $this->db->update("$table",array("$field"=>'inactive'),array('sch_id'=>$_SESSION['sch_id']));
            }
        }
         $query = $this->db->update("$table",array("$field"=>$status),array($key=>$id));
         if($query){
            return TRUE;
         }else{
            return FALSE;
    ...
    public function get_registered_subject($type,$session_id='',$student_id=''){
        $sch_id = $_SESSION['sch_id'];
        if(empty($session_id)){
         $session_id = $this->input->post('session_id');
        }
        ...
        $this->db->join('acadah_student_subjects',"acadah_student_subjects.student_id = acadah_student.student_id and acadah_student_subjects.subject_id = '$subject_id' and acadah_student_subjects.session_id='$session_id' and acadah_student_subjects.sch_id='$sch_id'");
        $this->db->from('acadah_users');
    ...
    public function get_unregistered_student($class_id,$subject_id,$session_id){
        $sch_id = $_SESSION['sch_id'];
        $this->db->select("student_id");
        $this->db->from("acadah_student_subjects");
        $this->db->where(array('subject_id'=>$subject_id,'session_id'=>$session_id));
    ...
    public function get_staff_registered_subject(){
        ...
        $this->db->where('acadah_staff_subjects.session_id',$session_id);
        $query = $this->db->get()->result_array();
        return $query;
    }

    //Function To Save Any Settings To the School Config Table
    //Collects To parameters The Type and Module Id
    public function save_settings($module_id, $type){
        $sch_id = $_SESSION['sch_id'];
        $names = array_keys($_POST);
            foreach($names as $name){
                $count = $this->get_details('acadah_school_config', 'key', $name,TRUE,TRUE);
                $data = array(
                    'key' => $name,
                    'value' => $_POST[$name],
                    'sch_id' => $sch_id,
            ...
            $this->db->trans_begin();
            if($count >0){
         $this->db->update('acadah_school_config',$data,array('sch_id'=>$sch_id,'key'=>$name));
            }else{
                $this->db->insert('acadah_school_config', $data);
            }
            $tran_id = $this->db->insert_id();
            }
             if ($this->db->trans_status() === FALSE){
            ...
            if($type == 'general' and $query === TRUE){
            //Change The MOdule Setting Status To Completed
            $this->db->update('acadah_school_modules',array('settings_status'=>'completed'),array('sch_id'=>$sch_id,'module_id'=>$module_id));
        }
        return $query;
    }

    public function ts_details($ts_id = '',$term_id='',$session_id=''){
        $sch_id = $_SESSION['sch_id'];
        if(!empty($ts_id)){
            $query = $this->db->get_where('acadah_app_term_session',array('ts_id'=>$ts_id)); 
        }else{
            $this->db->join('acadah_school_term',"acadah_school_term.term_id = acadah_app_term_session.term_id and acadah_school_term.sch_id='$sch_id'");
            $query = $this->db->get_where('acadah_app_term_session',array('acadah_app_term_session.term_id'=>$term_id,'session_id'=>$session_id));
        }
        // die ($this->db->last_query());
        return $query->result_array();
