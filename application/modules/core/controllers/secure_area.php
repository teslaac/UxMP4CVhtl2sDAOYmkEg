...
// echo '</pre>';
		...
		// }
		if($module_id != NULL){
		$module_settings = $this->Corem->get_details('acadah_school_config', 'module_id', $module_id,TRUE);

		foreach ($module_settings as $mod_setting) {
			$setting[$mod_setting['key']] = $mod_setting['value'];
		}
		// print_r($setting);
		if(!empty($setting)){
		$data['module_settings'] = $setting;
	}
		...
		$data['school_details'] = $this->Corem->get_school_details();
		$this->load->vars($data);
