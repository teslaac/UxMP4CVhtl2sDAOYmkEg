...
class Core extends Secure_area {
	...
	public function status($table,$id,$status,$field='status',$url){
		$status = $this->Corem->status($table,$id,$status,$field);
		if($status === TRUE){
	...
	public function get_unregistered_student(){
		...
		$students = $this->Corem->get_unregistered_student($class_id,$subject_id,$ts_id);
			foreach($students as $student){
            echo "<option value='$student[student_id]'>". $student['student_id'].'-'. $student['lname'].' '.$student['fname'] . "</option>";
       	 }
